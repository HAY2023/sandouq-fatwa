...
-- Add countdown_title column to settings
ALTER TABLE public.settings ADD COLUMN IF NOT EXISTS countdown_title text DEFAULT 'حلقة الإفتاء ستكون بعد';

-- Update the update_settings_authenticated function to include countdown_title
...
BEGIN
  ...
  UPDATE settings SET
    ...
    next_session_date = CASE WHEN p_next_session_date IS NOT NULL THEN p_next_session_date::timestamp with time zone ELSE next_session_date END,
