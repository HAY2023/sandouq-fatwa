...
DROP FUNCTION IF EXISTS public.update_settings_authenticated(text, boolean, timestamp with time zone, text, text, boolean, boolean, boolean);
DROP FUNCTION IF EXISTS public.update_settings_authenticated(text, boolean, timestamp with time zone, text, text, boolean, boolean, boolean, boolean, text);

-- إعادة إنشاء الدالة بالشكل الصحيح
