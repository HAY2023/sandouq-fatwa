...
-- Drop all overloads of update_settings_authenticated
DROP FUNCTION IF EXISTS public.update_settings_authenticated(text, boolean, text, text, text, boolean, boolean, boolean, boolean, text, boolean, integer, text, text, text, text);
...
-- Recreate single unified function with ALL parameters
...
DECLARE
  v_valid boolean;
BEGIN
  SELECT verify_admin_password(p_password) INTO v_valid;
  IF NOT v_valid THEN
    RETURN false;
  END IF;
